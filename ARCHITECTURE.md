...
| `api/radar.js` | Cached, bounded dashboard snapshot adapter | Vercel OIDC, official BigQuery Gold and optional enrichment |
...
The scheduled production flow is `Cloud Scheduler → Workflows → Cloud Run Job (explicit UTC window) → official Bronze + arXiv Silver → Dataform (Hugging Face Silver, assertions, Gold) → independent enrichment Job`. Scheduler runs ingestion at 00:20 UTC for the preceding completed UTC day and runs an independent Gold health check at 01:00 UTC. The enrichment Job runs only after core success; its failure or quota state is caught and cannot invalidate a Gold snapshot. The Job retains successful Bronze captures when either source fails and does not invoke Dataform for a partial source run. The two sources are new public Hugging Face model repositories and first arXiv submissions in `cs.AI`, `cs.CL`, and `cs.LG`. GitHub is out of scope as a data source.
